...
- `baseline: v2.5.5` —— 上一个已发布版本（v2.5.5 同版本修订于 2026-07-26 追加更新检测修复，`打包` 时作为 `git diff` 完整性校验的起点）
...
<!-- 上一个版本 v2.5.5 已发布（含 2026-07-26 同版本修订）。新的改动完成后在此追加一行：`- <emoji> <一句话描述>` -->
- ✨ 连接新增「高级选项」：强制 IPv4/IPv6 地址族、每连接连接超时、SSH Keepalive 间隔（仅 SSH/SFTP）
