...
- `baseline: v2.12.0` —— 上一个已发布版本（`打包` 时作为 `git diff` 完整性校验的起点）
...
<!-- 上一个版本 v2.12.0 已发布。新的改动完成后在此追加一行：`- <emoji> <一句话描述>` -->

- 🐛修复 AI 面板新增模型后选择器不刷新、需重启才能选到的问题：打开选择器即拉取最新列表，且头部标签与下拉高亮正确显示当前选中的具体模型
