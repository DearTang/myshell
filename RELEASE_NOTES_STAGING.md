...
- `baseline: v2.5.5` —— 上一个已发布版本（`打包` 时作为 `git diff` 完整性校验的起点）
...
<!-- 上一个版本 v2.5.5 已发布。新的改动完成后在此追加一行：`- <emoji> <一句话描述>` -->
- 🐛 修复 应用内检查更新始终检测不到新版本：Gitee 的 `/releases/latest` 接口不可靠（一直返回 v1.11.2），改用列表接口 `/releases` 并在客户端用版本比较选出真正的最新版
