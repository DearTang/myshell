...
- `baseline: v2.7.0` —— 上一个已发布版本（`打包` 时作为 `git diff` 完整性校验的起点）
...
<!-- 上一个版本 v2.7.0 已发布。新的改动完成后在此追加一行：`- <emoji> <一句话描述>` -->
- ✨新增空闲自动锁定：无操作超过设定时长（10分钟/30分钟/1小时可选，默认30分钟）后自动锁定应用，需重新输入密码；可在设置→安全中配置或禁用
