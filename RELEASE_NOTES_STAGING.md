...
- `baseline: v2.6.1` —— 上一个已发布版本（`打包` 时作为 `git diff` 完整性校验的起点）
...
<!-- 上一个版本 v2.6.1 已发布。新的改动完成后在此追加一行：`- <emoji> <一句话描述>` -->
- 🐛 服务器主机密钥变更后连接报含糊的 "Unknown server key" 且无法恢复：报错改为明确列出新旧指纹，错误屏「重新连接」按钮变为「重置密钥并重连」，一键自动清除旧记录并重新信任（仅 SSH/SFTP，保留 MITM 防护）
