...
- `baseline: v1.9.1` —— 上一个已发布版本（`打包` 时作为 `git diff` 完整性校验的起点）
...
- ✨新增 左下角反馈入口：提交反馈（描述+图片+自动附脱敏日志）到邮箱（Web3Forms）
- 🔒安全 日志脱敏：主机名/用户名/IP 在记录前掩码，反馈日志二次脱敏
<!-- 上一个版本 v1.9.1 已发布。新的改动完成后在此追加一行：`- <emoji> <一句话描述>` -->
