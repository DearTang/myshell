...
- `baseline: v2.6.0` —— 上一个已发布版本（`打包` 时作为 `git diff` 完整性校验的起点）
...
<!-- 上一个版本 v2.6.0 已发布。新的改动完成后在此追加一行：`- <emoji> <一句话描述>` -->
- 🐛 服务器主机密钥变更后连接报含糊的 "Unknown server key" 且无法恢复：现报错明确列出新旧指纹，并新增连接右键「重置主机密钥信任」自助恢复（仅 SSH/SFTP，保留 MITM 防护）
