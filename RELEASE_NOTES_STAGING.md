...
- `baseline: v2.9.0` —— 上一个已发布版本（`打包` 时作为 `git diff` 完整性校验的起点）
...
<!-- 上一个版本 v2.9.0 已发布。新的改动完成后在此追加一行：`- <emoji> <一句话描述>` -->
- ✨ 多窗口模式：在终端底部工具栏点击🪟多窗口，选择多个会话以网格布局同时展示，支持设置行列数、自动最大化、滚动溢出、超限确认
- ✨ 会话管理增强：当前会话下拉新增全部重连/全部广播/每行广播开关，广播组下拉新增退出全部广播
- 🛠️ CommandBar 窄屏自适应：窗口宽度不足时按钮只显示图标不显示文字，防止输入框挤压
