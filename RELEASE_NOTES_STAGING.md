...
- `baseline: v2.11.0` —— 上一个已发布版本（`打包` 时作为 `git diff` 完整性校验的起点）
...
<!-- 上一个版本 v2.11.0 已发布。新的改动完成后在此追加一行：`- <emoji> <一句话描述>` -->

- 🛠️优化 快捷命令多行执行支持行间延迟，避免下一条命令在交互提示/密码输入就绪前被提前送出：三种模式——`##delay:500`（或 `##delay:1s`）行内精确等待、固定延迟、**智能等待**（监听终端输出，等上一行输出静止后再发下一行，自动适配命令速度、能处理 `sudo`/`mysql` 密码提示）；在「设置 → 快捷命令」配置
