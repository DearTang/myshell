...
### 阶段 90：修复 SSH 应用级保活防不住 TMOUT 自动登出 + 自动锁定后台失效（2026-07-30）

- **需求：** 连接编辑「高级首选项」勾选了应用级保活，SSH 会话仍被服务器踢掉，报 `timed out waiting for input: auto-logout`（`[Connection closed]`）。
- **根因（TMOUT）：** `TMOUT`（服务器 `/etc/profile` 等设置）计的是**交互登录 shell 的键盘空闲时间**；而原保活实现是「另开一条独立 exec 通道跑 `true`」——它跑在**另一个非交互进程**里，不会重置登录 shell 的 `TMOUT` 计时器。即 SSH 连接与保活心跳一直活着，但交互 shell 自己把自己踢出去了。误用工具：exec 心跳只防 NAT/防火墙空闲断连（网络层），防不了 shell 层 TMOUT。
- **讨论：方案选型（重要决策）。** 一度考虑「A+B 组合」=定期往交互 PTY 注入字节 + 注入 `TMOUT=0`。但用户提出关键质疑：**编辑文件时注入字节会污染 vim/nano**——vim 插入模式下 NUL（`<C-@>`）会「重复上次插入文本并退出插入模式」，直接往文件里塞内容，比被踢更糟。结论：**没有哪种周期性 PTY 字节注入对 vim/nano 绝对安全**，A 方案否决。
- **实现（TMOUT，纯方案 B）：** `ssh.rs` `channel_reader` 开头（shell 刚就绪、用户首次输入前——此时前台进程必是登录 shell，绝不可能是 vim）向交互 PTY 一次性注入 `\nexport TMOUT=0 2>/dev/null\n`：`2>/dev/null` 吞掉 `readonly TMOUT` 环境的报错，避免刷屏；readonly 极端场景放弃保活（不牺牲编辑安全）。**保留**原 exec 心跳不动——它防的是 NAT/防火墙，对编辑器无干扰，两机制各管各的。
- **实测回归修复①（TMOUT 误伤未保活连接）：** 首版注入是无条件的，导致「只对一台勾了保活，两台都清了 TMOUT」。改为**仅当 `app_keepalive_secs > 0` 才注入**——未勾保活的连接保持服务器默认 TMOUT 行为，把 TMOUT 清零绑定到用户的显式 opt-in。
- **实测回归修复②（自动锁定后台失效）：** 用户反馈「窗口最小化/切到别的窗口超过半小时没自动锁定」。根因：自动锁定原实现是**裸 `setTimeout`**，窗口失焦后 OS webview（WebView2/WebKit）会像浏览器一样**节流/挂起后台定时器**，回调到点不触发，锁永远不触发。修复：`App.tsx` 锁定 effect 加**基于墙钟时间戳的兜底**——记 `lastActivityAt`，监听 `visibilitychange`/`focus`/`pageshow`，窗口重回前台时比较「距上次活动是否已超阈值」，超了**立即补锁**。后台 timer 即使被 throttle，窗口一恢复前台就能补上。`setTimeout` 仍是前台主驱动，兜底只在后台→前台切换时触发。
- **关于「自动锁定 vs 保活」：** 二者不冲突。锁定只丢弃内存 DEK + 盖 UI（`main.rs` lock_vault 单行 `*slot = None`），**不 disconnect、不关会话、不碰保活**；保活在后端 tokio 定时器里独立运行，锁定期间连接/心跳照常，解锁后原 tab 复用存活会话。锁定监听的是本地鼠标键盘，与 SSH 保活完全解耦。
- **涉及文件（2 个）：** `src-tauri/src/ssh.rs`、`src/App.tsx`。
- **验证：** `cargo check` + `npx tsc --noEmit` 均通过。

## 五问重启检查（阶段 90）
| 问题 | 答案 |
|------|------|
| 我在哪里？ | 阶段 90 complete —— 注入 `TMOUT=0`（已加 app_keepalive_secs 门控）防 shell 自动登出；自动锁定加时间戳兜底防后台 throttle；保留 exec 心跳防 NAT；cargo check + tsc 通过。 |
| 我要去哪里？ | 实测：①只对一台勾保活，另两台 `echo $TMOUT` 应保持原值；②勾保活那台静置超 TMOUT 不再被踢、vim 无污染；③把窗口最小化超半小时（或临时设 10 分钟），恢复前台应立即补锁。 |
