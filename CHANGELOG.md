...
- **多窗口模式**：在终端底部工具栏点击「🪟 多窗口」按钮，选择多个已连接的 SSH 会话，以网格布局同时展示在当前界面中。每个窗口可独立操作，终端历史完整保留（不重建实例）。支持设置中配置默认行列数（默认 2 行 × 3 列），进入多窗口时自动最大化窗口，超出容量的窗口可向下滚动查看。选择窗口数超过容量时弹出自定义确认提示。窗口数不足一行时自动压缩布局并顶满高度（如 2 个窗口→1×2 顶满，4 个→2×2 均衡排列）。
- **会话管理增强**：右上角「当前会话」下拉面板新增「全部重连」（一键重连所有掉线会话）、「全部广播」（一键将所有已连接 SSH 会话加入广播组）、每行会话右侧的广播开关（📡）。「广播组」下拉面板新增「退出全部广播」按钮。

#### 🛠️ 优化

- **CommandBar 窄屏自适应**：多窗口小窗格中 CommandBar 宽度不足时，右侧按钮自动只显示图标不显示文字，防止输入框被挤压。所有按钮加了 `flexShrink: 0` 确保宽度统一。

## v2.9.0（2026-07-30）

#### ✨ 新增

- **防止 shell 空闲自动登出（TMOUT）**：连接高级选项新增独立开关。勾选后登录时自动执行 `export TMOUT=0`，阻止服务器按空闲时间（TMOUT）自动登出。不额外开通道，受限单会话账号（堡垒机）也可用，对 vim 等编辑器无干扰。

#### 🛠️ 优化

- **移除「应用级保活（exec 心跳）」功能**：原功能每 N 秒新开 exec 通道跑 `true`，会打死受限单会话账号（堡垒机 MaxSessions=1）的连接。russh 自带 15 秒连接级 keepalive 对普通服务器已足够保持长连接，TMOUT 防护由上面的独立开关覆盖，故移除该功能以消除误用陷阱。

#### 🐛 修复

- **窗口最小化/失焦后自动锁定不触发**：窗口最小化或切到其它窗口超过设定时长，应用未自动锁定。根因为后台 webview（WebView2/WebKit）会像浏览器一样节流/挂起后台定时器，锁定的 setTimeout 到点不触发。现新增基于墙钟时间戳的兜底：窗口恢复前台时检查距上次活动是否已超阈值，超了立即补锁。
- **编辑连接后点标签页「重连」仍用旧配置**：编辑连接设置（如开启 TMOUT 防护）后，点标签页「重连」不生效，需从侧边栏重新打开才用新配置。根因为重连用的是开标签页时缓存的旧配置快照。现重连改为优先读取最新连接配置，与侧边栏重开行为一致。

## v2.8.1（2026-07-30）

#### 🐛 修复

- **SSH 应用级保活防不住 TMOUT 自动登出**：勾选「应用级保活」后，SSH 仍被服务器以 `timed out waiting for input: auto-logout` 踢出。根因为原保活是另开一条独立 exec 通道跑 `true`，它跑在另一个非交互进程里，不会重置登录 shell 的 `TMOUT` 计时器——即连接与心跳一直活着，交互 shell 却自己把自己踢了。现改为：仅对该连接，在交互 shell 启动时一次性注入 `export TMOUT=0`（仅作用于勾选保活的连接，不误伤其它会话；对 vim 等编辑器无干扰）；exec 心跳保留，继续防 NAT/防火墙空闲断连。
- **窗口最小化后自动锁定不触发**：窗口最小化或切到其它窗口超过设定时长，应用未自动锁定。根因为后台 webview（WebView2/WebKit）会像浏览器一样节流/挂起后台定时器，锁定的 setTimeout 到点不触发。现新增基于墙钟时间戳的兜底：窗口恢复前台时检查距上次活动是否已超阈值，超了立即补锁。

## v2.8.0（2026-07-29）

#### ✨ 新增

- **空闲自动锁定**：无操作超过设定时长后自动锁定应用，需重新输入密码解锁。可在设置→安全中配置时长（10 分钟 / 30 分钟 / 1 小时），也可选择「不启用」（首次解锁后不再自动锁定，重启恢复）。默认 30 分钟。

## v2.7.0（2026-07-29）

#### ✨ 新增

- **SFTP 传输取消**：下载/上传过程中可随时点击「取消」中止传输，后端在 32 KB 块边界即时响应，无需等待整个文件传完。

#### 🛠️ 优化

- **SFTP 传输进度面板重做**：仿 ZMODEM 四行布局（方向+文件名+取消 / 进度条 / 字节+百分比+速度 / 开始时间+已用+预计剩余），1 秒刷新驱动实时 ETA。
- **ZMODEM 进度面板增强**：新增开始时间、已用时间、预计剩余时间显示，刷新间隔改为 1 秒。

#### 🐛 修复

- **ZMODEM 下载大文件卡死**：>100 MB 文件传输约 130 MB 后完全卡死。根因为每个 8 KB 子包都触发一次 IPC 写入，消息队列饱和。改为 256 KB 缓冲批量写入，IPC 调用量降低 32 倍。
- **ZMODEM 原生下载文件大小不一致**：大文件落盘后比源文件多出数万字节。根因为 CRC 不完整时 payload 已被写入 pending_write，下次 feed 重复解码同一子包。修复：CRC 完整性检查提前到 payload 写入之前。
- **ZMODEM 取消后终端打印垃圾**：取消传输后 lrzsz 的 CAN 突发、错误文本、ZFERR 帧全部渲染到终端。改为 500 ms 时间窗口抑制，会话结束后短暂丢弃所有协议尾字节。

## v2.6.2（2026-07-27）

#### 🐛 修复

- **服务器主机密钥变更后无法连接**：服务器重装或重新生成密钥后，连接会报含糊的 `SSH connect failed: Unknown server key` 且无任何入口可恢复（首次信任本身正常，实为主机密钥变更触发了防中间人拦截）。现改为：报错明确列出本地记录与服务器当前的指纹，错误屏的「重新连接」按钮变为「重置密钥并重连」，一键自动清除旧记录并重新信任（仅 SSH/SFTP）。保留主机密钥变更的 MITM 防护，不自动接受变更后的密钥。

## v2.6.1（2026-07-27）

#### 🐛 修复

- **服务器主机密钥变更后无法连接**：服务器重装或重新生成密钥后，连接会报含糊的 `SSH connect failed: Unknown server key` 且无任何入口可恢复（首次信任本身正常，实为主机密钥变更触发了防中间人拦截）。现改为：报错明确列出本地记录与服务器当前的指纹，并在连接 ⋯ 菜单新增「重置主机密钥信任」（仅 SSH/SFTP，二次确认），重置后下次连接重新信任。保留主机密钥变更的 MITM 防护，不自动接受变更后的密钥。

## v2.6.0（2026-07-26）

#### ✨ 新增

- **连接高级选项（SSH/SFTP）**：连接对话框新增「高级选项」组，支持三项按连接配置——**地址族**（自动 / 强制 IPv4 / 强制 IPv6，用于绕开 IPv6 黑洞导致的连接卡顿）、**连接超时**（默认 10s）、**SSH Keepalive 间隔**（默认 15s）。默认值与原内置行为一致，不影响既有连接；SFTP 复用 SSH 会话自动生效。
